<?php
class tarifas_publicidad extends seccion{
    function index(){
        $db=dbInstance();
        //publiidad top
        $psql="select * from wee_publicidad inner join wee_estaticos
                    on wee_publicidad.id=wee_estaticos.pubtop where date(now()) between wee_publicidad.fechaini and wee_publicidad.fechafin and wee_publicidad.estado=1 and wee_estaticos.id = 2";
        $params1["publicidad"]=$db->loadObjectRow($psql);
        //articulo
        $sql="select * from wee_estaticos where estado=1 and id=2";
        $params2["articulo"]=$db->loadObjectRow($sql);
        //footer
        $params3=$this->do_footer();
        //obteniendo publicidad por defecto
        $sql="select banners from wee_estaticos where wee_estaticos.estado=1 and id=2";
